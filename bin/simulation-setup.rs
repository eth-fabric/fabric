...
"#,
	...
	fn cb_config(&self, gateway: bool) -> String {
		...
		let keys_path = if gateway { self.config.gateway_keys_path.clone() } else { self.config.proposer_keys_path.clone() };
		let secrets_path = if gateway { self.config.gateway_secrets_path.clone() } else { self.config.proposer_secrets_path.clone() };
		let proxy_key_dir = if gateway { self.config.gateway_proxy_key_dir.clone() } else { self.config.proposer_proxy_key_dir.clone() };
		format!(
			r#"# This file is automatically generated by the simulation-setup binary
