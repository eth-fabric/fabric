...
pub fn get_delegation_signing_root(delegation: &Delegation) -> Result<B256> {
    ...
    Ok(keccak256(
        (MessageType::Delegation.to_uint256(), delegation_evm).abi_encode_params(),
    ))
}
...
pub fn get_constraints_message_signing_root(constraints: &ConstraintsMessage) -> Result<B256> {
    ...
    let constraints_message_evm = SolConstraintsMessage {
        ...
        receivers: constraints
            .receivers
            .iter()
            .map(convert_pubkey_to_g1_point)
    ...
    Ok(keccak256(
        (
            MessageType::Constraints.to_uint256(),
            constraints_message_evm,
        )
            .abi_encode_params(),
    ))
}
...
fn get_signed_registration_sol_type(
    ...
    let signed_registration = SolSignedRegistration {
        pubkey,
        signature,
...
pub fn abi_encode_urc_register_inputs(inputs: &URCRegisterInputs) -> Result<Bytes> {
    let registrations = inputs
        .registrations
        .iter()
        .map(get_signed_registration_sol_type)
        .collect::<Result<Vec<_>>>()?;

    let register_call = SolRegisterCall {
        registrations,
        owner: inputs.owner,
        signingId: inputs.signing_id,
    };
...
mod tests {
    ...
    fn test_get_delegation_signing_root() -> Result<()> {
        ...
        assert_eq!(
            get_delegation_signing_root(&delegation)
                .unwrap()
                .to_string(),
    ...
    fn test_get_constraints_message_signing_root() -> Result<()> {
        ...
        assert_eq!(
            get_constraints_message_signing_root(&constraints_message)
                .unwrap()
                .to_string(),
