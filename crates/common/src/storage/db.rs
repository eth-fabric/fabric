...
impl DatabaseContext {
	...
	pub fn multi_get_raw<'a>(&self, keys: impl IntoIterator<Item = &'a [u8]>) -> Result<Vec<Option<Vec<u8>>>> {
		let mut out = Vec::new();
		for key in keys {
...
mod tests {
	...
	fn batch_write_and_multi_get_work() -> Result<()> {
		...
		let ops = vec![
			DbOp::Put { key: b"k1".to_vec(), value: b"v1".to_vec() },
			DbOp::Put { key: b"k2".to_vec(), value: b"v2".to_vec() },
			DbOp::Put { key: b"k3".to_vec(), value: b"v3".to_vec() },
		];
		...
		db.batch_write_raw([DbOp::Delete { key: b"k2".to_vec() }])?;
	...
	fn typed_json_roundtrip() -> Result<()> {
		...
		let value = MyValue { a: 42, b: "hello".into() };
	...
	mod dummy_constraints_crate {
		...
		fn extension_trait_roundtrip_works() -> Result<()> {
			...
			let c = DummyConstraint { id: 7, payload: "hello-constraints".to_string() };
