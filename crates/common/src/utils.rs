...
pub fn decode_pubkey(public_key: &str) -> Result<BlsPublicKey> {
    ...
    let public_key_bytes = public_key
        .try_into()
        .map_err(|e| eyre::eyre!("Failed to convert BLS public key to bytes: {:?}", e))?;
...
pub fn decode_address(address: &str) -> Result<Address> {
    ...
    let address_bytes = address
        .try_into()
        .map_err(|e| eyre::eyre!("Failed to convert address to bytes: {:?}", e))?;
...
mod tests {
    ...
    fn test_decode_pubkey_valid() {
        ...
        let pubkey_hex = "a0b0c0d0e0f0a1b1c1d1e1f1a2b2c2d2e2f2a3b3c3d3e3f3a4b4c4d4e4f4a5b5c5d5e5f5a6b6c6d6e6f6a7b7c7d7e7f7";
