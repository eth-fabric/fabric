...
use crate::constants::{PROPOSER_DUTIES_ROUTE, VALIDATOR_STATUS_ROUTE};
use crate::types::{
    BeaconApiConfig, ProposerDutiesResponse, ValidatorData, ValidatorDuty, ValidatorInfo,
    ValidatorResponse,
};
...
impl<H: HttpClient> std::fmt::Debug for BeaconApiClient<H> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BeaconApiClient")
...
impl<H: HttpClient> BeaconApiClient<H> {
    ...
    pub async fn get_proposer_for_slot(&self, slot: u64) -> Result<Option<ValidatorDuty>> {
        ...
        for duty in duties.data {
            let duty_slot = duty
                .parse_slot()
                .context("Failed to parse slot from validator duty")?;
            if duty_slot == slot {
                return Ok(Some(duty));
            }
        }
    ...
    fn parse_validator_info(data: &ValidatorData) -> Result<ValidatorInfo> {
        ...
        let validator_index = data
            .index
            .parse::<u64>()
            .context("Failed to parse validator index")?;
        ...
        T: for<'de> Deserialize<'de>,
    {
        ...
        if response.status != 200 {
            let error_text = String::from_utf8(response.body.clone())
                .unwrap_or_else(|_| "Unknown error".to_string());
            eyre::bail!(
                "Beacon API request failed with status {}: {}",
                response.status,
                error_text
            );
        }
...
mod tests {
    ...
    fn test_client_creation_with_short_timeout() {
        ...
        assert!(
            client.is_ok(),
            "Should create client even with short timeout"
        );
    ...
    fn test_client_creation_with_no_fallbacks() {
        ...
        assert!(
            client.is_ok(),
            "Should create client even with no fallbacks"
        );
    ...
    fn test_config_validation() {
        ...
        let error_msg = format!("{}", client.unwrap_err());
        assert!(
            error_msg.contains("Request timeout must be greater than zero"),
    ...
    fn test_client_clone() {
        ...
        assert_eq!(
            client.config.primary_endpoint,
            cloned_client.config.primary_endpoint
        );
        assert_eq!(
            client.config.fallback_endpoints,
            cloned_client.config.fallback_endpoints
        );
        assert_eq!(
            client.config.request_timeout_secs,
            cloned_client.config.request_timeout_secs
        );
    }
    ...
    fn test_parse_validator_info_active_ongoing() {
        ...
        assert!(
            result.is_ok(),
            "Should successfully parse active_ongoing validator"
        );
    ...
    fn test_parse_validator_info_active_exiting() {
        ...
        assert!(
            info.is_active,
            "Validator with active_exiting should be considered active"
        );
    ...
    fn test_parse_validator_info_active_slashed() {
        ...
        assert!(
            info.is_active,
            "Validator with active_slashed should be considered active"
        );
    ...
    fn test_parse_validator_info_exited_slashed() {
        ...
        assert!(
            !info.is_active,
            "Exited slashed validator should not be active"
        );
        assert!(info.is_slashed, "Validator should be marked as slashed");
    ...
    fn test_parse_validator_info_withdrawal_possible() {
        ...
        assert!(
            !info.is_active,
            "Validator in withdrawal_possible should not be active"
        );
        assert!(!info.is_slashed);
    ...
    fn test_parse_validator_info_invalid_index() {
        ...
        assert!(
            error_msg.contains("Failed to parse validator index"),
            "Error should mention validator index parsing"
        );
    }
    ...
    async fn test_get_proposer_for_slot_found() {
        ...
        mock_client.expect_get().times(1).returning(move |url| {
            ...
            Ok(HttpResponse {
                status: 200,
                body: serde_json::to_vec(&duties_response).unwrap(),
            })
        });
    ...
    async fn test_get_proposer_for_slot_not_found() {
        ...
        let empty_duties = ProposerDutiesResponse {
            execution_optimistic: false,
            finalized: true,
            data: vec![],
        };
        ...
        mock_client.expect_get().times(1).returning(move |_url| {
            Ok(HttpResponse {
                status: 200,
                body: serde_json::to_vec(&empty_duties).unwrap(),
            })
        });
        ...
        assert!(
            result.is_ok(),
            "Should successfully query even if no duty found"
        );

        let duty = result.unwrap();
        assert!(
            duty.is_none(),
            "Should return None when no duty exists for the slot"
        );
    }
    ...
    async fn test_get_proposer_for_slot_wrong_slot_in_epoch() {
        ...
        assert!(
            duty.is_none(),
            "Should return None when requested slot doesn't match any duty"
        );
    }
    ...
    async fn test_primary_fails_fallback_succeeds() {
        ...
        assert!(
            result.is_ok(),
            "Should succeed with fallback endpoint after primary fails"
        );
    }
    ...
    async fn test_all_endpoints_fail() {
        ...
        mock_client
            .expect_get()
            .times(2)
            .returning(|_url| Err(eyre::eyre!("Network error")));
    ...
    async fn test_network_timeout_simulation() {
        ...
        mock_client
            .expect_get()
            .times(1)
            .returning(|_url| Err(eyre::eyre!("Request timeout after 30 seconds")));
    ...
    async fn test_http_403_forbidden() {
        ...
        mock_client.expect_get().times(1).returning(|_url| {
            Ok(HttpResponse {
                status: 403,
                body: b"Forbidden - Rate limit exceeded".to_vec(),
            })
        });
