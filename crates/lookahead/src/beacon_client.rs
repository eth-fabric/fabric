...
use crate::types::{
	BeaconApiConfig, ProposerDutiesResponse, ValidatorData, ValidatorDuty, ValidatorInfo, ValidatorResponse,
};
...
impl<H: HttpClient> BeaconApiClient<H> {
	...
	pub async fn get_validator_status(&self, validator_pubkey: &BlsPublicKey) -> Result<ValidatorInfo> {
		...
		// Try primary endpoint
		match self.make_request::<ValidatorResponse>(&self.config.primary_endpoint.to_string(), &endpoint).await {
		...
		// Try fallback endpoints
		for fallback_endpoint in &self.config.fallback_endpoints {
			match self.make_request::<ValidatorResponse>(fallback_endpoint.to_string().as_str(), &endpoint).await {
				Ok(response) => {
					debug!(
						endpoint = %fallback_endpoint,
						pubkey = %pubkey_hex,
...
mod tests {
	...
	fn create_test_config() -> BeaconApiConfig {
		BeaconApiConfig {
			...
			fallback_endpoints: vec![Url::parse("https://beacon-nd-123-456-789.p2pify.com").unwrap()],
	...
	fn test_fallback_endpoint_order() {
		...
		assert_eq!(client.config.fallback_endpoints[0], Url::parse("https://fallback1.test").unwrap());
		assert_eq!(client.config.fallback_endpoints[1], Url::parse("https://fallback2.test").unwrap());
		assert_eq!(client.config.fallback_endpoints[2], Url::parse("https://fallback3.test").unwrap());
	}
	...
	fn test_client_creation_preserves_endpoint_urls() {
		...
		let config = BeaconApiConfig {
			...
			fallback_endpoints: vec![Url::parse(fallback1).unwrap(), Url::parse(fallback2).unwrap()],
			request_timeout_secs: 10,
			genesis_time: 1606824023,
		};
		...
		assert_eq!(client.config.fallback_endpoints[0], Url::parse(fallback1).unwrap());
		assert_eq!(client.config.fallback_endpoints[1], Url::parse(fallback2).unwrap());
	}
