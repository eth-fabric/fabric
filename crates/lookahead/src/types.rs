...
impl ValidatorDuty {
	...
	pub fn parse_slot(&self) -> Result<u64> {
		Ok(self.slot.parse::<u64>().map_err(|e| eyre::eyre!("Failed to parse slot: {:?}", e))?)
	}

	pub fn parse_validator_index(&self) -> Result<u64> {
		Ok(self.validator_index.parse::<u64>().map_err(|e| eyre::eyre!("Failed to parse validator index: {:?}", e))?)
	}
}
