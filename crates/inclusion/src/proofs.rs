...
impl InclusionProof {
    ...
    pub fn from_bytes(bytes: &Bytes) -> Result<Self> {
        let proof: InclusionProof = bincode::deserialize(bytes.as_ref())
            .wrap_err("failed to deserialize InclusionProof")?;
        Ok(proof)
    }
}
...
impl TransactionTrieBuilder {
    ...
    pub fn build(transactions: &[TxEnvelope]) -> Result<Self> {
        ...
        for (idx, tx) in transactions.iter().enumerate() {
            ...
            builder
                ...
                .wrap_err_with(|| {
                    format!("Failed to insert transaction at index {idx} into trie")
                })?;
    ...
    pub fn root(&mut self) -> Result<B256> {
        let root = self
            .trie
            .root_hash()
            .wrap_err("Failed to compute trie root hash")?;
    ...
    pub fn get_proof(&mut self, tx_index: usize) -> Result<Vec<Vec<u8>>> {
        ...
        let proof = self.trie.get_proof(key.as_slice()).wrap_err_with(|| {
            format!("Failed to generate proof for transaction at index {tx_index}")
        })?;
        Ok(proof)
    }
    ...
    pub fn verify_proof(&self, tx_index: usize, proof: &[Vec<u8>], root: &B256) -> Result<Vec<u8>> {
        ...
        self.trie
            ...
            .wrap_err_with(|| {
                format!("Failed to verify proof for transaction at index {tx_index}")
            })?
    ...
    pub fn get_tx_hash(&self, tx_index: usize) -> Result<B256> {
        self.transactions
            .get(tx_index)
            .copied()
            .ok_or_else(|| eyre!("Transaction not found at index {tx_index}"))
    }
}
